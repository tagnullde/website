---
title: "HTB-Writeup - Irked"
date: 2019-04-27
# bookComments: false
# bookSearchExclude: false
---

{{< banner "Irked-Banner.png" "HTB Irked machine info card: Linux, Easy difficulty, 20 points" >}}
...
This runs all the default labeled nmap scripts, enumerates versions of found services
...
So let's start exploring what else we've got so far.
...
Regular ssh service. Nothing special so far. We get prompted for a password.
<br>
<br>
#### Port 80:
Let's look at the Website:

![Angry emoji website hinting IRC is almost working](IrkedWebsite.png)

This looks like an obvious hint. "IRC is almost working".
Usually IRC runs on ports: "194/tcp" or "6667/tcp". We haven't seen those ports yet.
...
So I searched for its intended use, and exploits of it. Long story short: It's
...
Eventually I resorted to the HackTheBox forums and checked the thread for this box.
One guy said:
...
And there we have it. "Port 6697" is open and looks like IRC is listening.

Let's check if this is indeed IRC listening here. So I got myself an IRC client.
In this case "HexChat" and connected to the server.

![HexChat server settings for port 6697](IrkedIRCSetting.png)

![HexChat console showing UnrealIRCd 3.2.8.1 version](IrkedIRCVersion.png)
...
Which typically means, we need to privesc to the other user first. Let's check both
...
for the user.txt which is right next to the backup file.
(User.txt isn't accessible for us as you can see.)

However I wasn't sure what to do with it. "Steg" is a reference for "Steganography"
that's for sure. But there's nowhere to put the password. I searched everywhere in the filesystem.
After quite some time I took a break and thought:

> Where is typically some stego hidden? Well, pictures. But I don't have a picture.
> ...except the one on the Website!

Damn! This took me way too long. So, what next? I downloaded the picture and installed a famous
...
Awesome. We are not only authenticated as a proper user, we also can
...
Let's run the program and check what it does.
