---
title: "HTB-Writeup - Fortune"
date: 2019-08-31
# bookComments: false
# bookSearchExclude: false
---

{{< banner "fortune-banner.png" "HTB Fortune machine info card: Other OS, Insane difficulty, 50 points" >}}
...
![Web form to select a fortune database](http_fortune_tool.png)

You can select a fortune-database as it seems and get a fortune-cookie.

![Fortune cookie result page with quote text](http_fortune_cookie.png)

To make the writeup easier to follow we'll explore "port 443" later.
In reality I checked "port 443" at this point to get a high level overview before exploring "port 80" further.

To see what's happening in the background - let's send the request to "burp".

![Burp request showing the db=fortunes parameter](http_burp_01.png)

As you can see, with our selection on the site, we filled the `db=` parameter.
...
However, I wasn't able to spawn a "reverse-shell" this way. So I spent some time enumerating the filesystem.
...
![Firefox SSL unknown CA error message](https_firefox_error.png)
...
![Chromium error for missing client certificate](https_chromium_error.png)
...
![Chromium dialog to select the client certificate](https_chromium_select_cert.png)
...
![AuthPF page showing generated SSH key pair](https_chromium_authpf.png)

I created an "id_rsa" file for "ssh" and logged in.

![SSH login authenticated as nfsuser](ssh_nfsuser_auth.png)
...
![Masscan output listing newly discovered open ports](masscan_ports.png)
...
To make my life easier I added a proper ssh-key into charlie's account.
I just created a "ssh-key", added the "public-key" to his ".ssh/authorized_keys" file and logged into "ssh".

`ssh -i .ssh/id_rsa charlie@10.10.10.127`

# Internal Recon
According to Bob's Email. He worked on "pgadmin4" let's locate the database and see if we can mess with it.
...
![Database keys table with secret key values](database_keys.png)

![Database users table with password hashes](database_user.png)
...
script online on pastebin. At this point I was working on this machine for about three days and I couldn't be bothered to
...
Which was importing the crypto-lib from "github" and changing the proper values from the database.

You can find the finished script here: [decrypt.py](decrypt.py).
...
![HTB Certified badge earned for Fortune](fortune_badge.png)