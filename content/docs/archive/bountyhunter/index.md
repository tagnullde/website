---
title: "HTB-Writeup - BountyHunter"
date: 2021-11-20T17:54:29+02:00
# bookComments: false
# bookSearchExclude: false
---

{{< banner "bountyhunter-banner.png" "HTB BountyHunter badge: a goblin bounty hunter holding a wanted poster" >}}
...
We find an ssh service on port 22 and an http service on port 80. http is usually a better target than ssh. So I start there.
The website boasts a bounty hunter service. First I click on all visible links and menus to get a feeling for the website.
...
![Portal under development page with link to the bounty tracker](portal.png)

It's under development. An indicator that not all features (like proper coding) are production ready.
The "Go here" link leads us to a bounty report system which seems to be in a beta stage.

![Bounty Report System beta submission form](bounty-report-system.png)
...
![Bounty report form echoing back the submitted data](bounty-system-nice-data.png)
...
![Burp Suite intercepted POST request with base64-encoded data](burp-1.png)

Interesting: We send the data to a page with a funky name:

```
POST http://10.10.11.100/tracker_diRbPr00f314.php
```

The next interesting piece of information is the custom `X-Header`:
...
![Burp decoder revealing the base64-decoded XML payload](burp-2.png)

So we do in fact have XML formatted data. Seeing this, my gut feeling is, we should try `XML-Entity-Injection` or `XXE`.
...
We know it's a linux box, so `/etc/passwd` is a good candidate. Furthermore, I made an `/etc/hosts` entry for that box, so I can use a hostname instead of an IP. It helps sometimes.
...
![Burp response showing /etc/passwd leaked via XXE](burp-3.png)
...
I tried reading his `ssh` key, but wasn't successful. Maybe our `gobuster` search in the background can help us?
...
We have three files with a status code of 200. The last one `db.php` sounds very promising. If we adapt our proof of concept above to read `db.php` we will
get nothing. This could be because the file is not in the current working directory or more likely because it is a .php file. It gets interpreted from the webserver and
won't display its content to us. But we can use a php-filter in our XXE attack to encode the data to base64 first. 
...
![Decoded db.php source revealing database credentials](burp-4.png)
...
![Terminal output of sudo -l showing ticketValidator.py runnable as root](user-sudo.png)
...
The script reads a file with a ".md" extension and checks if the string ```# Skytrain Inc``` is in the first line of that file. Next it checks if the string ```## Ticket to ``` is on the second line.
...
![Terminal output of the ticketValidator script accepting a valid ticket](ticket-poc.png)
...
work. But it looks nicer. So the next iteration of the ticket file will have "development" in the "Ticket to" field.
...
A couple of lines earlier we can see that the script gets the ticket code by splitting the fourth line at a "+" symbol. So, if we provide a python command, in the fourth line after the ticket code,
...
![Terminal output of the ticket exploit triggering the reverse-shell command](exploit.png)

![Netcat listener receiving a root reverse shell](root.png)
