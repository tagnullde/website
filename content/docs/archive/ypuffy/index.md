---
title: "HTB-Writeup - Ypuffy"
date: 2019-02-09
# bookComments: false
# bookSearchExclude: false
---

{{< banner "ypuffy-Banner.png" "HTB Ypuffy machine info card: Other OS, Medium difficulty, 30 points" >}}
...
The output is quite big. You can find the output [here](nmap.txt).
...
![LDAP entry for user alice showing sambaNTPassword hash](alice-hash.png)

As this is very juicy information. I paused my enumeration and tried to exploit this.
But how?

Password hashes are often used as a substitute to passwords. Which means, you don't
need the original password to authenticate to a service. Just the hash. This is how
your corporate single-sign-on stuff works. Otherwise you would have to log in every time.
So it's a convenience thing.

The method is called Pass-The-Hash. And of course there are tools out there, that help
you with this. Like: `pth-smbclient`. If you don't have it, just go to github and get the `pth-toolkit`.

# Exploit
After fiddling around with the options, syntax and where to connect to I managed to connect to the alice
share:
...
![pth-smbclient session listing my_private_key.ppk on alice share](smb-connect-alice.png)

After downloading the file, and trying to use it as a key to access ssh I got an error.
This format is not compatible with ssh. So, I tried converting it. A quick google search helped
me find the correct command for this task. But first we need to install putty-tools.
...
![SSH shell as alice1978 with user.txt listed](shell-alice.png)
...
This is something you only get by googling or being up-to-date with infosec news.
...
This seems to work so far. But can we log in now?
