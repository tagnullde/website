---
title: "HTB-Writeup - Cap"
date: 2021-10-03
# bookComments: false
# bookSearchExclude: false
---

{{< banner "cap-banner.png" "HTB Cap badge: a pirate captain illustration" >}}
...
![Searchsploit results listing vsftpd exploits](searchsploit.png)

Nothing for vsftp. OpenSSH looks quite new but I suggest checking it as well.
I just made a mental note to come back to SSH should other paths seem to be a dead end.
Gunicorn had no version number, so I skipped the searchsploit as well. But we can take a look at the website itself.
...
![Dashboard sidebar menu with Security Snapshot and Network Status options](menu.png)
...
![Security dashboard showing zero captured network packets](networktraffic.png)
...
![File dialog for downloading the 3.pcap capture file](download.png)

Let's check if there's a "2.pcap". And there is. Albeit empty too.

![File dialog for downloading the 2.pcap capture file](download2.png)

There's even a "0.pcap". And this one has indeed data in it.
Let's download it and open it with wireshark.

![Wireshark capture revealing plaintext FTP username and password](creds.png)
...
Nathan probably reused his password, and SSH is far more interesting than FTP. And if it doesn't work, we can come back to FTP.

![Terminal showing SSH login as user nathan](nathan.png)
...
![Terminal output of downloading linpeas.sh with wget](linpeas.png)
