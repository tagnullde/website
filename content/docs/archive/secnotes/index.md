---
title: "HTB-Writeup - SecNotes"
date: 2019-01-19
# bookComments: false
# bookSearchExclude: false
---

{{< banner "SecNotes-Banner.png" "HTB SecNotes machine info card: Windows, Medium difficulty, 30 points" >}}
...
Spoiler alert: I found Port 8808 to be open also.
...
##### Port 80
We find a login page. I first tried to login with the usual suspects.

![Secure Notes login page with username and password fields](SecNotes-Login.png)
...
![Sign-up form for creating a new Secure Notes account](SecNotes-SignUp.png)

That worked - let's login.

![Notes page for user x41 with GDPR banner showing tyler's email](SecNotes-LoggedInAsUser.png)
...
Correct it's the name "tyler" in the email-address. Perhaps this could be useful later.
...
So I started up "gobuster" and found a file called "db.php". Based on the name "db" it is assumed
...
I won't give you all the things I tried. I spent at least an hour or two but nothing came up.
So I was stuck. After checking the HackTheBox forums for a hint, someone mentioned that this type
...
So I watched it again just to learn it was indeed about "SQLi". Specifically "2nd order SQLi".
You should watch (and subscribe!) his video for a better explanation on this type of "SQLi".
...
![Sign-up form with a single quote entered in the username field](SecNotes-SQLi-Test.png)

After logging in we get an error!

![500 Internal Server Error page shown after login](SecNotes-500Error.png)
...
![Sign-up form with a SQL comment injection payload in the username](SecNotes-SQLi-2.png)

Nice - we are able to login again. But wait, something is wrong here:

![Notes page displaying another user's note via SQL injection](SecNotes-SQLi-Success.png)

We can see the note from the legitimate "x41" User. We can read other users' content!
...
the already existing user account "x41" (without the single-quote).
...
![Login form using a SQL injection payload to authenticate as tyler](SecNotes-SQLi-Tyler.png)

And after logging in we find something nice in his notes. :)

![Tyler's notes revealing SMB share credentials for new-site](SecNotes-Tyler-Creds.png)

Now we can try to connect to the SMB share with those credentials.

And there's the webroot of the IIS Server.

![File browser showing IIS webroot files on the new-site SMB share](SecNotes-SMB-Share.png)

The note says, this is supposed to be the new website.
The old one is based on "php". Perhaps PHP is working there too?
...
![Directory listing returned by the uploaded PHP webshell](SecNotes-RCE.png)
...
![Netcat listener receiving a Windows reverse shell connection](SecNotes-RevShell.png)
...
I had to go back to the forums again to seek out help. Where someone said:

"What OS Version have you found - there's a specific new feature in this."

Because I couldn't think about anything from the top of my head, I googled for the release notes.
...
After that I used "wsl.exe" and "wsl.config" to start up the preconfigured "Ubuntu" Machine.
...
I had to search a lot, after a little nudge I headed in the right direction.
...
![Bash history revealing the Administrator account's SMB password](SecNotes-AdminPW.png)

# Root Flag
From here it just was connecting to the SMB share to get the root flag.
I didn't bother getting a proper shell as admin, which I leave up to the reader how
to do that. ;) (Hint: Impacket)

![Administrator desktop folder containing the root.txt flag](SecNotes-root.txt.png)

# Lessons Learned
First, my "Enumeration" failed me and I didn't follow my initial thoughts about the Ubuntu folders.
