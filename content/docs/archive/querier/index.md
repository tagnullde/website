---
title: "HTB-Writeup - Querier"
date: 2019-06-21
# bookComments: false
# bookSearchExclude: false
---

{{< banner "querier_banner.png" "HTB Querier machine info card: Windows, Medium difficulty, 30 points" >}}
...
Especially when I see "port 445" open. "Port 445" is used for "SMB" or filesharing on Windows.

But this time the "nmap" scripts failed to identify anything useful on this port. A dead end?
...
I used the file explorer to connect as an anonymous user.

![GNOME Connect to Server dialog for anonymous SMB share](smb_connect_01.png)

![Currency Volume Report.xlsm file found in SMB share](smb_connect_02.png)
...
![LibreOffice warning dialog about document macros](xlsm_macro_warning.png)

Let's investigate the macro.

![VBA macro code containing SQL connection credentials](xslm_macro_content.png)
...
![mssqlclient.py connected to SQL Server as reporting user](sql_access.png)
...
But, of course there's a way around that. By pure chance I watched a video of ippsec who had the very same problem in a box earlier that week.
...
With `xp_dirtree` I can list folders and shares. And as Windows loves to do "single-sign-on" for the user with its "NTLM" hash
...
![Responder capturing NTLMv2-SSP hash for mssql-svc](responder_ntlm.png)
...
Almost. I seem to have the proper permissions now. But I need to enable it.
...
This looks promising. My next goal should be to get a proper "reverse-shell" and some tools for the next step on the box.
...
![xp_cmdshell copying powerup.ps1 via esentutl LOLBIN](download_lolbin.png)
...
![Netcat receiving Windows command shell connection](reverse_shell.png)

# Privilege Escalation
First, I've got the `user.txt`.

![Directory listing showing user.txt flag file](user_flag.png)

This time, I haven't spent time in a dedicated "internal-recon" phase.
It was a mixed bag due to the tools I used. The mentioned "powerup.ps1" script does technically both:
...
![PowerUp Invoke-AllChecks output showing UsoSvc and GPP password](powerup_checks.png)
...
# Root Flag
I used "powerup.ps1" again, to exploit the "UsoSvc" service.
...
![whoami output showing nt authority\system shell](root_shell.png)
