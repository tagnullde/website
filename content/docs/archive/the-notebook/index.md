---
title: "HTB-Writeup - The Notebook"
date: 2021-07-31
# bookComments: false
# bookSearchExclude: false
---

{{< banner "notebook-banner.png" "HTB The Notebook badge: an operator wearing a headset in front of monitoring screens" >}}
...
The most interesting service so far is http on port 80. Let's take a peek there.

![The Notebook web app homepage](website-inital.png)
...
![Browser storage tab showing auth JWT token](auth.png)
...
![JWT decoder showing kid header and admin_cap payload](jwt-decoded.png)

On the top-right we can see the header. And it already leaked some interesting information. The parameter `kid` or `key-id` holds the information about the private-key used to sign this token.

On the bottom-right we see our login data. And as the `admin_cap` parameter states, we are not an admin. We would like to change that though.
We can edit the token and change the 0 to a 1. But that would invalidate the signature. At least if the application in the backend checks the signature. After playing with the token a bit, it seemed like it does check the signature. So we have to find a way to sign the token with a valid key.
...
![Python web server receiving request for privKey.key](poc.png)
...
![Admin panel access after forging JWT token](admin-panel.png)
...
![Netcat reverse shell connected as www-data](low-shell.png)

We have a shell. Let's upgrade our shell to a proper tty with `python3 -c 'import pty;pty.spawn("/bin/bash")'` then `ctrl-z` to background the shell. Then type `stty raw -echo` and hit enter
...
He just managed to set up his backup. With `find / -type d -name "*backup*"` we can search for a folder named "backup" and will show a folder where we have access to: `/var/backups`.

In there is a tar.gz backup of noah's home directory. We will copy it to `/tmp` and unpack with `tar -xvzf`. Inside we find a noah's ssh-key. A quick `chmod 600` on it is enough to make it work for us.

![SSH login as user noah confirmed via id](ssh.png)

Here we also find the user.txt.

# Privilege Escalation

Now it's time to dig deeper and find a way to get root. Let's start with the basics: `sudo -l`.

![sudo -l output showing docker exec permission](sudo.png)
...
This is an old version. After a small google search we can assume this docker version is vulnerable to `CVE-2019-5736`.
...
In the second terminal window you will ssh into the box with noah again and then trigger the exploit via `sudo /usr/bin/docker exec -it webapp-dev01 /bin/sh`.
...
![Root shell obtained via Docker CVE exploit](root.png)

And that's it. We've rooted a medium difficulty machine once again. 

# Bonus Content

As promised here's an alternative way to mess with the token. First we need to create a key-pair with ssh-keygen.
