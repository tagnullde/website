---
title: "Exploit-Dev - Stack Overflow"
date: 2019-08-08
# bookComments: false
# bookSearchExclude: false
---

{{< banner "oscp-banner.png" "Offensive Security OSCP exploit-dev banner" >}}
...
I just know a bit of "Python3" and "PowerShell". But I managed to pull this off anyway and so can you.
...
This part should be straightforward for you. Just install both operating systems and make sure that the VMs can "ping" each other.
...
With this, we should have everything set up.
...
![nmap scan showing SLMail POP3 port 110 open](nmap-scan.png)
...
![netcat session authenticating to POP3 with USER and PASS](nc-pop3.png)
...
# Fuzzing
At some point a developer wrote the "POP3" server in a programming language like "C". In those languages the developer is responsible
...
![Meme: wife asks about a problem, husband blames a small buffer](meme.jpg)
...
Nothing prevents you to send even more data: "200 Bytes", "2000 Bytes" - the application will take it and put all this data into its memory.
...
and then the `PASS` command with junk data. For simplicity I chose the letter "A". Please check the comments in the code to get an overview of what it does:
...
![OllyDbg process selection dialog listing SLMail.exe](ollydbg-attach.png)

![OllyDbg attached to SLMail process disassembly view](ollydbg-attached.png)

While "OllyDbg" is now monitoring the process, we can start the "Fuzzer".

![Terminal output of fuzzer script sending increasing buffer sizes](fuzzer-start.png)
...
![OllyDbg full CPU view after crash with EIP set to 41414141](ollydbg-crash1.png)

Let's zoom in a bit and take a look at the different windows and their content.
On the right hand-side you can see the "registers" of the "CPU". "Registers" are small units of memory within the "CPU" itself.

![Registers panel showing EIP overwritten with 41414141](ollydbg-crash1-registers.png)

The "CPU" loads data from "RAM" into those "registers" and then processes the data.
We are just interested in two "registers" right now. "EIP" and "ESP" (the latter is highlighted).

"EIP" is the "Extended Instruction Pointer". It tells the "CPU" where it needs to go after it is
done with the current operation. Currently it is pointing to the memory-location of `41414141`.

"ESP" is the "Extended Stack Pointer". It always points to the top of the "stack".
In this example the top of the "stack" is located at `0193A101`.

Wtf is the "stack"?
We can see the "stack" in the bottom-right corner.

![Stack dump filled with repeating 41414141 (AAAA) bytes](ollydbg-crash1-stack.png)

It is basically just the "RAM" allocated for the "SLMail" process. The screenshot shows that the content of the "stack" at that location is also "41's". It also shows, what "41" means. It's our character "A" we have sent in "hex" notation.

Our "A's" have overwritten "EIP" and crashed the program.  

![OllyDbg status bar showing access violation reading 41414141](ollydbg-crash1-accessviolation.png)
...
The "return" address is the location where the application under normal circumstances would continue its operation. In this case probably doing a check
if the provided user and password are correct. But because we overwrote the "return address" with `41414141` (an invalid address) the program crashes.
...
![Terminal output of pattern_create generating a 5000-byte pattern](pattern-create.png)
...
![pattern_offset finding the crashed EIP value at offset 4654](pattern-offset.png)
...
![Stack dump confirming A's, B's, and C's at correct offsets](poc-stack.png)

![Registers panel showing EIP overwritten with 42424242](poc-eip-42.png)

And indeed it worked.
The next step in our process is to find so called "bad characters".

# BadChars
"BadChars" are bytes that the application can't handle and would break our exploit. A prime example is so called "null bytes" or `\x00`.
...
Finding them is an easy but repetitive task. We swap our "C's" with all possible bytes from `\x00` till `\xff` and see which break the program.
When we found one, we'll remove it from the list, and try again. And again. And again.

![Exploit code sending buffer with the full badchar byte range](poc-badchars.png)

After the first run, we see nothing on the "stack". So we found our first "badchar" `\x00`.

![Stack dump showing buffer truncated after the null byte](poc-badchar1.png)
...
![Stack dump showing the complete badchar byte sequence intact](poc-badchar2.png)

With this knowledge we can move into the next phase. Creating our "shellcode".

# Shellcode
"Shellcode" is basically a synonym for "payload". It can be anything you want.
...
The payload is an unstaged "reverse_tcp shell". The attacker machine's "IP" and "port" and last but not least I specified the "BadChars".
Those will be avoided by "msfvenom" when generating the "shellcode".  The output looks like this:

![msfvenom terminal output generating the encoded reverse shell payload](poc-shellcode.png)
...
The question is now: How can we access our "shellcode"? Well, we control "EIP" and could put the address in there where our "shellcode" starts. However in most cases you
...
To prove that I run the newest version of the "exploit" again.
...
![Registers panel showing EIP still overwritten after adding shellcode](poc-esp-register.png)

![Stack dump showing ESP pointing at the start of the shellcode](poc-esp-stack.png)
...
You can read a good explanation <a href="https://security.stackexchange.com/questions/157478/why-jmp-esp-instead-of-directly-jumping-into-the-stack" target="_blank">here</a>.
...
![OllyDbg dialog searching SHELL32.dll for a JMP ESP instruction](jmp-esp.png)

![Disassembly showing a JMP ESP instruction found in SHELL32.dll](jmpesp-found.png)
...
Let me show you the finished "exploit" and then see if we can pop a shell.
...
![Terminal showing reverse shell connection as nt authority\system](exploit-success.png)

And it worked! :)

In the next screenshot you can see how the "stack" and "registers" look just one step before we jump into our `NOP`s.

![OllyDbg view with EIP at JMP ESP and NOP sled on the stack](exploit-ollydbg-final.png)
...
didn't even have a `JMP ESP` or `CALL ESP` instruction in it. Don't know why. But that's why I learn, right?! ;)
...
I played a bit with "Immunity Debugger" in combination with the extension "mona.py" instead of "OllyDbg".
...
![Immunity Debugger memory map showing JMP ESP in SLMFC.dll's .rdata section](jmpesp-found2.png)
