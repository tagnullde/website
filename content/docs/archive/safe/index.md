---
title: "HTB-Writeup - Safe"
date: 2019-10-24
# bookComments: false
# bookSearchExclude: false
---

{{< banner "safe-banner.png" "HTB Safe machine info card: Linux, Easy difficulty, 20 points" >}}
...
In the very long output of this scan we can find this line commented out in the source-code...
...
![HTML source comment revealing the myapp download and port 1337](safe-comment-myapp.png)

# Explore

Let's see if "Port 1337" is indeed open:

![Netcat connected to port 1337 showing the echo prompt](nc-port-1337.png)
...
![Browser dialog downloading the myapp binary](safe-myapp-download.png)
...
![checksec output showing NX enabled, no canary, no PIE](safe-checksec.png)
...
In order to overcome this, we need to use an exploitation-technique known as "ROP - Return Oriented Programming".
...
![ROPStar tool output automatically spawning a shell](safe-ropstar-shell.png)
...
most of the 32Bit <a href="https://ropemporium.com/" target="_blank">ROPEmporium</a> Challenges which are dedicated to teaching <kbd>ROP</kbd> techniques.
...
First we need to crash the program to check if it's vulnerable. I didn't use a special "fuzzer" for that.
I just tried a couple of inputs until I found a crash. I used 300 Bytes to generate the crash in the screenshot.

![Terminal crashing with a segmentation fault after long input](safe-segfault.png)
...
produced a pattern that I couldn't use to find the offset. I suspected bad characters and tried to create a custom pattern-set. Didn't work either. If you know what's going on here - let me know.

`gdb-peda$ pattern_create 300 pattern.txt`

Next we start "myapp" with "gdb-peda" and throw our pattern in.

![gdb-peda loading myapp with the pattern file](safe-fuzz.png)
...
For now we just use the <kbd>p64()</kbd> function as a helper to convert the addresses we will use for our "ROPChain" to "little endian" and also to the proper byte sequence.
...
![gdb-peda running myapp with the crafted payload file](safe-poc.png)

Success. "RIP" has been overwritten with a value we control.

![gdb-peda register dump showing RIP overwritten with 0x42424242](safe-poc-ripoverwrite.png)
...
Let's get an overview of the functions in the program. "objdump" is the right tool for that.
...
![objdump disassembly of the main() function](safe-obj-main.png)

And here's another interesting function called <kbd>test()</kbd>.

![objdump disassembly of the test() function](safe-obj-test.png)
...
![ropper output listing available pop gadgets](safe-ropper-pop.png)

Our "grep" already filtered the output for a couple of "pop gadgets".
But which one should we use? Well, we need to jump to the register in which we will store the <kbd>system()</kbd> address. So let's start with finding a <kbd>jmp</kbd>
...
![Disassembly of test() highlighting the jmp r13 instruction](safe-jmp-r13.png)
...
![ropper gadget popping r13, r14, r15 before ret](safe-pop13.png)
...
![Disassembly showing the mov rdi, rsp gadget](safe-obj-rdirsp.png)

Last but not least, we need the memory address of <kbd>system()</kbd>.

![Disassembly showing the call to system() at 0x401040](safe-mem-system.png)
...
The exploit code will be copied into the buffer, will do its thing and start up a shell. But, because the "stdin" will be closed now (no more data is in the text file)
...
![Local exploit spawning a root shell via the cat pipe trick](safe-exploit-local-working.png)
...
![Remote exploit over netcat spawning a shell on the target](safe-exploit-remote-working.png)

We are in! :)
And we also have access to the `user.txt`.

![Listing /home/user showing photos, KeePass database, and user.txt](safe-userflag.png)
...
Keepass, like many other password managers, has a feature where you can enter your password and a keyfile.
...
To download the files, we first need better access. Let's deploy an "ssh-key" on the box and login with a real shell.

First we generate a new key and get the public part of it:

![ssh-keygen generating a new SSH key pair](safe-sshkeygen.png)

Then we "echo" it over into the <kbd>/home/user/.ssh/authorized_keys</kbd> file.

![Appending the public key to authorized_keys](safe-authkeys.png)
...
![KeePassX master key dialog with password and keyfile](safe-keepass.png)

If you don't have keepass installed on your box <kbd>apt install keepassx</kbd> will do the trick.

A simple <kbd>su -</kbd> gets us the root flag.

![Switching to root via su and listing root.txt](safe-root.png)
