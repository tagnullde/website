---
title: "Exploit-Dev - ret2plt"
date: 2020-04-21
# bookComments: false
# bookSearchExclude: false
---

{{< banner "ret2plt_banner.png" "Debugger register dump with the ret2plt technique name overlaid" >}}
...
need to run properly. For example the `printf()` or `puts()` function. Once a program is started the libc library is dynamically linked into that program.
Which means the program could use all the functions in libc.

To find the function, let's say `printf()`, within libc the program needs to know what the memory location of the function is at runtime. 
...
One solution to this problem could be to host multiple copies of `printf()` for each process. But, as you can imagine - that's quite inefficient.
...
If this explanation is too much for you right now, I suggest taking a look at this [blog post](http://infosec.vishalmishra.in/2013/03/procedure-linkage-table-and-global.html).
The author has some nice pictures that might help you understand this a bit better.

The main takeaway should be: Functions are called with a level of indirection. A call to `printf()` will first call `printf@plt`.
...
Because of this randomization, we can't hardcode memory addresses into our exploits anymore. They will change once a process restarts.
What we can do though, is use the stub's addresses instead, which then resolve to the real function address we want to call.

Before you get even more confused, let's go and play with an example.

# The Target

This is our target program we will attack today.
...
![Program output asking 'Can you make me crash'](target-showcase-1.png)

Nice. An almost useless program.

## First Crash

The next part is of course to make it crash. We know from the source code that 100 bytes are the maximum
...
![Segmentation fault after sending 300-byte input](target-first-crash.png)

We confirmed it's crashing when a large input is given. The next step is to examine when the crash happened
exactly. If you are not familiar with this process you should read my [OSCP buffer-overflow](/docs/archive/stack-overflow) article. This is just the setup
we always need to do, regardless of technique used.

## Finding the Offset

To find the offset where the crash is happening I use gdb with the [gef extension](https://gef.readthedocs.io/en/master/).

{{< highlight bash >}}
gdb ./ret2plt
{{< /highlight >}}

Once gdb is up, I set a breakpoint in the main function.

{{< highlight bash >}}
b main
{{< /highlight >}}

![GDB with GEF loaded and breakpoint set at main](gdb-offset-1.png)
...
Hit `c` to continue. You should get the password-prompt now. Paste the pattern you've copied and hit enter.

As expected, the program crashed again.

![GEF register dump showing overwritten $rsp at ret](gdb-pattern-crash.png)
...
![GEF pattern search finding 120-byte offset](gdb-offset-found.png)
...
After importing pwntools we need to specify which program we want to run. We also set the os we are running currently
...
Most of them are found in libc. Which is not the target for today's mission. Scroll up and you will find
something like this:

![GDB grep results showing two 'sh' string addresses](sh-in-binary.png)
...
![objdump disassembly showing system@plt address](system-plt.png)

We are interested in the first line. This is the address of `system@plt`. This address is not randomized.
And this is also our stub to the real `system()`. Let's put that also into our exploit.

Are we good? Not yet. One last piece is missing. We are running all this on a 64 Bit system. Which means
that arguments to a function, like "sh" for `system()`, need to be passed via the `$rdi` register.

How do we get it in there? Glad you asked. As DEP is also enabled and we can't execute data from the stack anyways
we have to use a ROP technique as well. We need a `pop rdi; ret` instruction that's already present within our binary.
...
![ROPgadget output listing pop rdi; ret gadget](pop-rdi.png)
...
Let's run the exploit and check if a file called "payload" has been created. If so, let's run the following command:
...
![Shell obtained by piping payload into vulnerable binary](cat-shell.png)
...
Once we hit our breakpoint, single-step through the program with `s`. Every time you enter a function, you can rush through
it with `finish`. However, pay attention to the "code" section, once you step out of `scanf()`, we want to observe what's
happening.

![GEF disassembly stepping through scanf before finish](scanf-finish.png)

Let's go slower now.

Once we are back in `main()` we can see our payload on the stack.

![GEF stack view filled with A characters after scanf](full-stack.png)

Also we see that we are about to `leave` the `main()` function.

![Disassembly of main about to execute leave and ret](leave-main.png)
...
![Execution landing on pop rdi; ret gadget](pop_rdi-2.png)

Now check the stack. After the `pop rdi` comes `sh`. 

![Stack showing pop rdi gadget then address of 'sh'](pop-sh.png)
...
![Disassembly about to jump into system@plt](system-call.png)
...
Next we remove the write function and introduce the last two lines of this finished exploit code:
...
![Pwntools interactive shell running as root](final_shell.png)

# Last words

That's it for today. I hope this was somewhat understandable and helps you write your own exploits.
Of course you can download all the code from my [github](https://github.com/tagnullde/Exploit-Development/tree/master/ASLR-Bypass/ret2plt).

In the next days I would like to write another article that takes this a bit further. 
What happens if we don't have `system()` or "sh" at our disposal? What can we do then?
